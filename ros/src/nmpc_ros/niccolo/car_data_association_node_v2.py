...
class MappedCar:
    ...
    def __init__(self, car_id: int, x: float, y: float,
        ...
        self.id = car_id
        self.x, self.y = x, y
        self.tag = tag                    # "front" | "back" | "unknown"
...
class GpsRtkBackend:
    ...
    def _handle_heading(self, raw):
        ...
        try:
            ...
            return
        with self.lock:
            self.theta = math.radians(h - 90)     # NED -> ENU yaw [rad]
...
class DataAssociationNode:
    ...
    def _step_impl(self, rgb_msg: Image, depth_msg: Image):
        ...
        for car, car_depth, target, match_quality in best_by_id.values():
            ...
            # Row 1 is reserved for a future second value and is never
            # written here — it stays 0 for every car, always.
            ...
            rospy.loginfo(
                "[Assoc] car %d (%s): score=%+.2f (car=%.2f person=%s "
                "depth=%.2fm car_pos=(%.2f,%.2f) match_err=%.2fm)",
                target.id, target.tag, score_now, car_conf,
        ...
        for near_car in near_cars:
            ...
            expected_dist = near_car.distance_to(rx, ry)
            best_person_conf = None
            for p in persons:
