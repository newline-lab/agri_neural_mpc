...
from nmpc_ros.msg import Trajectory
...
class NeuralMPC:
    def __init__(self):
        ...
        self.neighbor_predictions = [None] * (n_robots+1) # init predictions
        subscribers_pred_net = []
        for neighbor in neighbors:
            topic = f"/agent_{neighbor}/predictions"
            sub = rospy.Subscriber(topic, Trajectory, self.prediction_callback)
            subscribers_pred_net.append(sub)
        # neighbors' positions
    ...
    def prediction_callback(self, msg):
        self.neighbor_predictions[msg.id] = np.array([msg.positions_x, msg.positions_y]).T
    ...
    # Utility Functions
    # ---------------------------
    def publish_trajectory(self, predicted_states):
        msg = Trajectory()
    ...
    def run_simulation(self):
        ...
        while mpciter < sim_time and not rospy.is_shutdown():
            ...
            if self.assigned is not None:
                ...
                self.publish_trajectory(x_traj)
